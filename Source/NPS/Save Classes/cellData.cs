    ...
    public class cellData : IExposable
    {
        ...
        public void doPack()
        {
            ...
            if (map.zoneManager.ZoneAt(location) is Zone_Growing &&
                (currentTerrain.defName != "TKKN_DirtPath" || currentTerrain.defName != "TKKN_SandPath"))
            {
                return;
            }
        ...
        private void clearLoot()
        {
            ...
            for (var i = things.Count - 1; i >= 0; i--)
            {
                ...
                if (plant.def.HasModExtension<ThingWeatherReaction>())
                {
                    ...
                    Log.Warning("Destroying " + plant.def.defName + " at " + location + " on " +
                                currentTerrain.defName);
                    plant.Destroy();
                }
                else
                {
