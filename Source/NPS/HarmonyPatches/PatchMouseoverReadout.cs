    ...
    internal class PatchMouseoverReadout
    {
        private static void Postfix()
        {
            ...
            if (Settings.showDevReadout)
            {
                ...
                var cell = watcher.cellWeatherAffects[c];
                rect = new Rect(BotLeft.x, UI.screenHeight - BotLeft.y - num, 999f, 999f);
                ...
                rect = new Rect(BotLeft.x, UI.screenHeight - BotLeft.y - num, 999f, 999f);
                var label4 = "Cell Info: Base Terrain " + cell.baseTerrain.defName + " Current Terrain " +
                             cell.currentTerrain.defName + " | Wet " + cell.isWet + " | Melt " + cell.isMelt +
                             " | Flooded " + cell.isFlooded + " | Frozen " + cell.isFrozen + " | Thawed " +
                             cell.isThawed + " | Getting Wet? " + cell.gettingWet;
                Widgets.Label(rect, label4);
                num += 19f;

                rect = new Rect(BotLeft.x, UI.screenHeight - BotLeft.y - num, 999f, 999f);
                var label6 = "TKKN_Wet " + cell.currentTerrain.HasTag("TKKN_Wet") + "TKKN_Swim " +
                             cell.currentTerrain.HasTag("TKKN_Swim");
                Widgets.Label(rect, label6);
                num += 19f;


                rect = new Rect(BotLeft.x, UI.screenHeight - BotLeft.y - num, 999f, 999f);
                var label5 = "Cell Info: howWet " + cell.howWet + " | How Wet (Plants) " + cell.howWetPlants +
                             " | How Packed " + cell.howPacked;
